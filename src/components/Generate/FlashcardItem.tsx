    ...
    >
      <div className="p-3 pb-2 flex items-center justify-between">
        <h3 className="text-xs font-medium text-muted-foreground">
          Fiszka #{index}
          {proposal.wasEdited && <span className="ml-1.5 text-xs text-primary">(edytowana)</span>}
          {proposal.wasAccepted && <span className="ml-1.5 text-xs text-green-600">(zaakceptowana)</span>}
        </h3>
        <div className="flex gap-1.5">
          <Button
            ...
            aria-label={`${proposal.wasAccepted ? "Odznacz" : "Zaakceptuj"} fiszkę ${index}`}
            className={cn(
              "h-7 w-7 p-0",
          ...
          <Button
            ...
            aria-label={`${isEditing ? "Anuluj edycję" : "Edytuj"} fiszkę ${index}`}
            className={cn(
              "h-7 w-7 p-0",
              isEditing
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between text-xs min-h-[16px]">
                ...
                {!frontValid && front.length > 0 && (
                  <span className="text-destructive">{front.length === 0 ? "Wymagane" : "Zbyt długie"}</span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1 flex-1 flex flex-col">
              ...
              <div className="flex justify-between text-xs min-h-[16px]">
                ...
                {!backValid && back.length > 0 && (
                  <span className="text-destructive">{back.length === 0 ? "Wymagane" : "Zbyt długie"}</span>
                )}
              </div>
            </div>
          </>
